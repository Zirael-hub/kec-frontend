        ...
        {Array.isArray(contact.socials) && contact.socials.length>0 && (
          <div className="flex flex-wrap gap-2 pt-1">
            {contact.socials.map((s:any, i:number)=>(
              <a key={i} href={s.url} target="_blank" className="rounded-xl bg-slate-50 px-3 py-1.5 text-xs font-medium ring-1 ring-black/5 hover:bg-slate-100 dark:bg-slate-800">
                {s.label}
              </a>
            ))}
          </div>
